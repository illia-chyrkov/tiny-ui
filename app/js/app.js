	...
	if ($(this).closest('.file-input__wrap-multiple').length > 0) {
		...
		Array.from(this.files).forEach(image => {
			...
			file.onloadend = (event) => {
				$(this).closest('.file-input__wrap').find('.file-input__preview').append(`
		...
		file.onloadend = (event) => {
			$(this).closest('.file-input__wrap').find('.file-input__preview-img').attr('src', event.target.result)
		}
	}
})
...
$('.input--autocomplete input').keyup(function(e){
	...
	$(this).closest('.input--autocomplete').find('.input--autocomplete__item').addClass('hidden')
	$(this).closest('.input--autocomplete').find('.input--autocomplete__item').each(function(){
